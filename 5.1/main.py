...
for i in moves:
        ...
        output.write("Move " + str(num_blocks) + ' from ' + str(from_idx) + ' to ' + str(to_idx) + "\n")

        # Move i blocks from one place to another, for each block, pop it off one, append to another 
        for i in range (num_blocks):
                cur_block = stacks[from_idx].pop()
                print(cur_block)
                stacks[to_idx].append(cur_block)
